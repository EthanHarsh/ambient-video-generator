...
export function readDefaultAudioFile() {
    return readAudioFile({directoryPath: join(__dirname, '../audio'), extension: '.wav'});
}

export function findDefaultAudioFile() {
    return findFileFromDirectoryByExtension({directoryPath: join(__dirname, '../audio'), extension: '.wav'});
}