...
export function getDirectoryContents(directoryPath: string) {
    return Effect.tryPromise({
        try: async () => {
            ...
            return fileNames.map(name => buildFileMetadata({directoryPath, name}));
        },
        catch: (unknown) => new Error(`Error reading files from ${directoryPath}: ${unknown}`)
    })
}
...
export function readFileFromDirectoryByExtension({directoryPath, extension}: ReadFileFromDirectory) {
...
export function findFileFromDirectoryByExtension({directoryPath, extension}: ReadFileFromDirectory) {
    return Effect.gen(function* () {
        const files = yield* getDirectoryContents(directoryPath);
        const file = yield* findFileByExtension({files, extension});
        return file.path;
...
export function findFileByExtension({files, extension}: FindFileByExtension) {
    return Effect.try({try: () => {
        ...
        return file;
    }, catch: (unknown) => new Error(`Error finding file with extension ${extension}: ${unknown}`)})
}

export function readFileByPath(filePath: string) {
    return Effect.tryPromise({
        try: () => readFile(filePath),
        catch: (unknown) => new Error(`Error reading file from ${filePath}: ${unknown}`)
    })
}